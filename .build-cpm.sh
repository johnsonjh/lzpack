...
PACK="${PACK:-1}"            # 1 = pack the .COMs with the host optimal (-e)
                             # self-extractor; 0 = leave them raw
TPA48="${TPA48:-0xBDFF}"     # 48K-system memory ceiling (= MEMTOP); the build
                             # fails if a tool's runtime footprint won't fit
WINMIN="${WINMIN:-1024}"     # smallest window lzpack will fall back to (matches
...
pack() {                     # $1 = .com
  before= after=
  [ "${PACK}" = 0 ] && return 0
  [ -x ./lzpack ] || { printf '%s\n' ">> host ./lzpack missing; not packing $1"; return 0; }
  before=$(wc -c < "$1")
...
check_48k() {                # $1=label  $2=.map  $3=extra heap (0=none)  $4=fatal?
  end= peak= ceil= avail= win=
  if [ ! -f "$2" ]; then printf '%s\n' ">> [$1] no map ($2); cannot size-check"; return 0; fi
  end=$(sed -n 's/^__BSS_END_head *= *\$\([0-9A-Fa-f]*\).*/\1/p' "$2" | head -1)
  if [ -z "${end}" ]; then printf '%s\n' ">> [$1] no __BSS_END in map; skip check"; return 0; fi
  ...
  if [ "${peak}" -gt "${ceil}" ]; then
    ...
    avail=$(( ceil - 0x${end} - 2048 - STACKSZ )); win=$(( avail / 3 ))
...
build_arch() {               # $1 = clib   $2 = outdir
  clib=$1; out=$2
  [ "${out}" = "." ] || mkdir -p "${out}"
  lc=${out}/lzpack.com; lm=${out}/lzpack.map
  sc=${out}/stubasm.com; sm=${out}/stubasm.map
