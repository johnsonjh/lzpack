...
cpm86 cpm-86: cs8080.h csz80.h cschk.h csmsg.h stubasm.c lzpack.c \
	...
	(upx -q -1 --8086 ./cpm-86/stubasm.cmd 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
	...
	(upx -q -1 --8086 ./cpm-86/lzpack.cmd 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
...
msdos dos pcdos: cs8080.h csz80.h cschk.h csmsg.h stubasm.c lzpack.c \
	...
	(cd msdos && owcc -v -bcom -march=i86 -mcmodel=t -frerun-optimizer \
		...
		-DNDEBUG ../stubasm.c)
	(upx -q -1 --8086 ./msdos/stubasm.com 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
	...
	(upx -q -1 --8086 ./msdos/lzpack.com 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
...
djgpp: cs8080.h csz80.h cschk.h csmsg.h stubasm.c lzpack.c
	...
	cat "$${CWSDSTUB:-/opt/cwspdmi/cwsdstub.exe}" ./djgpp/stubasm \
		> ./djgpp/stubasm.exe
	(upx -q -1 ./djgpp/stubasm.exe 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
	"$${DJGPP_GCC:-/opt/djgpp/bin/i586-pc-msdosdjgpp-gcc}" -s \
	...
	cat "$${CWSDSTUB:-/opt/cwspdmi/cwsdstub.exe}" ./djgpp/lzpack \
		> ./djgpp/lzpack.exe
	(upx -q -1 ./djgpp/lzpack.exe 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
...
windows: cs8080.h csz80.h cschk.h csmsg.h stubasm.c lzpack.c
	...
	(upx -q -1 windows/lzpack64.exe 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
	"$${MINGW32_GCC:-i686-w64-mingw32-gcc}" -O3 -s \
		-o ./windows/lzpack32.exe ./lzpack.c
	(upx -q -1 windows/lzpack32.exe 2> /dev/null | \
		grep ' \-> ' 2> /dev/null) || :
