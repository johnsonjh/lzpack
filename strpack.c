...
static int
name_in (const char *const *list, const char *p, size_t n)
{
  int i;
...
/* 1 = the packers compile the string, 0 = they do not, -1 = bad guard */
  ...
  while (0 != more)
    {
      ...
      else if (name_in (sp_def_no, g, nl))
        {
          if (0 == neg)
            carried = 0;
        }
...
static int
blob_verify (const unsigned char *coded, const unsigned char *plain,
             long plen)
{
  long o = 0;
